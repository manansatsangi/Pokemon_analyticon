...
# ### Instructions:
# * Wait for Versus mode to load
# * Select two pokemon to compare
# * Wait for update after selection
...
app_url = "https://pokemon-dash-1.onrender.com/"  # Replace with your actual URL

# Use HTML to apply custom CSS for controlling the width
iframe_html = f'<iframe src="{app_url}" style="width: 100%; max-width: 1200px; height: 600px; border: none;"></iframe>'
HTML(iframe_html)
...
# ### Pokemon Status analysis

# * The world of pokemon primarily comprises of **normal** pokemon. The **Mythical, Legendary and Sub legendary** are rare!
# * These pokemon are overall stronger than normal pokemon - have better **attack, defense and speed** stats!
# * They are also **taller and heavier** compared to other pokemon
# * These pokemon are also **more difficult to catch** as they have a lower catch rate (average) compared to Normal pokemon
# * The Mythical and Legendary pokemon are usually weak against **Ghost, Bug and Dark** pokemon. It would be a great idea to weaken these pokemon in battle.
...
# * Almost **53%** of pokemon have a secondary type.
# * Pokemon with secondary type **have better base stats** compared to those that don't.
# * These pokemon are also **difficult to catch**, relatively
...
base_stats('type_number')


# In[31]:


ptype("catch_rate","type_number")


# ### Pokemon variation by Generation

# * After 1st generation, the **5th generation** saw the introduction of highest number of pokemon. **Generation 6** saw the lowest.
# * In general, with each generation, the game saw introduction of pokemon with higher attack stat. The game has **progressive become attack-centered.**
# * **Generation 4 and 7** saw the introduction of pokemon that were relatively difficult to catch.
# * Pokemon introduced in **Generation 5** were the smallest and lightest.
...
stacked_barplot(df_poke, "generation", "status")
...
stacked_barplot(df_poke, "generation", "growth_rate")


# ### Pokemon Catch rate analysis

# In[45]:


# Histogram of catch_rate
plt.figure(figsize=(10, 6))
sns.histplot(data=df_poke, x='catch_rate', bins=20, kde=True)
...
plt.figure(figsize=(12, 10))
correlation_matrix = df_poke[['catch_rate', 'hp', 'attack', 'defense', 'sp_attack', 'sp_defense', 'speed',]].corr()
sns.heatmap(correlation_matrix, annot=True, cmap="viridis", center=0)
plt.title('Correlation Heatmap')
plt.show()
...
ptype("catch_rate","growth_rate")


# In[49]:


ptype("catch_rate","egg_type_1")
